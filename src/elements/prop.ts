...
const propElement = elementUtils.registerElement('ui-prop', {
  ...
  _initEvents(): void {
    ...
    this.addEventListener('mousemove', (event: Event) => {
      ...
      const newRatio =
        (resizeStartWidth + (e.pageX - resizeStartX)) / label.parentElement!.clientWidth;
      let minWidth = 80;
      ...
      if (
        (newRatio <= currentRatio || newRatio <= 0.8) &&
        newRatio * (label.clientWidth / currentRatio) > minWidth
      ) {
  ...
  attrsChanged: undefined as
    | ((oldAttrs: Record<string, any>, newAttrs: Record<string, any>) => void)
    | undefined,
