...
const resizableBehavior: ResizableBehavior &
  ThisType<HTMLElement & ResizableBehavior & ResizableElement> = {
  _resizable: true,
  ...
  _initResizable(): void {
    ...
    if (this._initMaxWidth !== 'auto' && (this._initMaxWidth as number) < this._initMinWidth!) {
      console.warn(
    ...
    if (this._initMaxHeight !== 'auto' && (this._initMaxHeight as number) < this._initMinHeight!) {
      console.warn(
      ...
      for (const child of resizableChildren) {
        ...
        if (hasAutoWidth || child._computedMaxWidth === 'auto') {
          ...
          this._computedMaxWidth =
            ((this._computedMaxWidth as number) || 0) + (child._computedMaxWidth as number);
        }
      ...
      for (const child of resizableChildren) {
        ...
        if (hasAutoHeight || child._computedMaxHeight === 'auto') {
          ...
          this._computedMaxHeight =
            ((this._computedMaxHeight as number) || 0) + (child._computedMaxHeight as number);
        }
      }
    }
