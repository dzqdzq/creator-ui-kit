 ...
 */
function collectFontAwesomeStyles() {
  ...
  css = css.replace(/@font-face\s*\{[^}]*font-family:\s*['"]?FontAwesome['"]?[^}]*\}/s, newFontFace);
 ...
 */
function generateFontFaceRule(themeName) {
  const fontFormats = [
    { ext: 'woff2', format: 'woff2', mime: 'font/woff2' },
    { ext: 'woff', format: 'woff', mime: 'font/woff' },
    { ext: 'ttf', format: 'truetype', mime: 'font/ttf' },
    { ext: 'eot', format: 'embedded-opentype', mime: 'application/vnd.ms-fontobject' }
  ];
  
  const srcParts = [];
  
  // 处理每种字体格式
  for (const { ext, format, mime } of fontFormats) {
    try {
  ...
  elementFiles.forEach(file => {
    ...
    if (elementName && !processedFiles.has(file)) {
      ...
      try {
        ...
        const needsFontello = COMPONENTS_USING_FONTELLO.includes(elementName) || 
    ...
    if (globalName && !processedFiles.has(file)) {
      ...
      try {
        ...
        if (globalName === 'common') {
          cssContent = cssContent.replace(/@font-face\s*\{[^}]*font-family:\s*fontello[^}]*\}/gs, '');
        }
 ...
 */
function extractFontFace(cssMap, themeName) {
