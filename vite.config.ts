...
function processThemeUrlsBuild(
  ...
  while ((match = urlRegex.exec(css)) !== null) {
    ...
    try {
      ...
      if (base64) {
        // 如果有片段标识符，保留它
        const fragment = themeUrl.includes("#")
          ? "#" + themeUrl.split("#")[1]
          : "";
        replacements.push({
          index: matchIndex,
  ...
  for (const { ext, format } of fontFormats) {
    try {
      const fontPath = resolve(
        __dirname,
        `src/themes/${themeName}/font/fontello.${ext}`
      );
...
function collectAllStyles(themeName = "default"): Record<string, string> {
  ...
  const fontFaceCss = generateFontFaceRule(themeName);
  const fontelloPath = resolve(
    __dirname,
    `src/themes/${themeName}/globals/fontello.css`
  );
      ...
      if (id.endsWith("css-loader.ts") || id.endsWith("css-loader.js")) {
        ...
        if (injectedCode === code) {
          ...
          injectedCode = code.replace(
            pattern2,
            "const compiledStyles = " + cssMapStr + ";"
          );
        }
        ...
        if (injectedCode === code) {
          ...
          injectedCode =
            "const __COMPILED_STYLES__ = " + cssMapStr + ";\n" + code;
        }
        ...
        if (fontFace) {
          ...
          injectedCode =
            "const __GLOBAL_FONT_FACE__ = " + fontFaceStr + ";\n" + injectedCode;
        }
